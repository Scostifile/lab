...
func (kv *ShardKV) GetCommandFromRaft(message raft.ApplyMsg) {
	...
	if op.Operation == NEWCONFIGOp {
		...
		if kv.maxraftstate != -1 {
			kv.IfNeedToSendSnapshotCommand(message.CommandIndex, 1)
		}
		return
...
func (kv *ShardKV) ExecuteNewConfigOpOnServer(op Op) {
	...
	for shard := 0; shard < NShards; shard++ {
		if kv.migratingShard[shard] != Normal {
			//fmt.Printf("%v: gid=%v rf.me=%v ExecuteNewConfigOpOnServer %v in configNum:%v newConfigShard=%v myConfigShard=%v\n",
			//	kv.me, kv.gid, kv.rf.Me(), kv.migratingShard, kv.config.Num, newConfig.Shards, kv.config.Shards)
			return
		}
	}
	...
	for shard := 0; shard < NShards; shard++ {
		...
		if oldShards[shard] != kv.gid && newShards[shard] == kv.gid {
			if oldShards[shard] != 0 {
				...
				fmt.Printf("%v: lockMigratingShardsL oldShards[%v]=0 in configNum=%v\n", kv.me, shard, kv.config.Num)
			}
			continue
		}
	}
...
func (kv *ShardKV) ExecuteMigrateShardsOnServer(op Op) {
	...
	for _, shardComponent := range op.MigrateData_MIGRATE {
		if kv.migratingShard[shardComponent.ShardIndex] == Normal {
			fmt.Printf("ExecuteMigrateShardsOnServer migrateShard = normal?\n")
			continue
		}
		...
		if kv.migratingShard[shardComponent.ShardIndex] == BePulling {
			...
			kv.migratingShard[shardComponent.ShardIndex] = GCing
		}
